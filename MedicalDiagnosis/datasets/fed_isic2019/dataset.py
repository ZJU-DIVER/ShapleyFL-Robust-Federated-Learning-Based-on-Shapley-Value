...
from MedicalDiagnosis.utils import check_dataset_from_config
...
if __name__ == "__main__":
    ...
    print(len(mydataset))
    print("Server Size of image 0 ", mydataset[0][0].shape)
    mydataset = FedIsic2019(train=False, pooled=True)
    print(len(mydataset))
    print("Server Test Size of image 0 ", mydataset[0][0].shape)
    total_test = 0
    total_train = 0
    for i in range(6):
        mydataset = FedIsic2019(center=i, train=True, pooled=False)
        print(len(mydataset))
        total_train += len(mydataset)
        print("Client Test Size of image 0 ", mydataset[0][0].shape)
        mydataset = FedIsic2019(center=i, train=False, pooled=False)
        print(len(mydataset))
        total_test += len(mydataset)
        print("Client Test Size of image 0 ", mydataset[0][0].shape)
