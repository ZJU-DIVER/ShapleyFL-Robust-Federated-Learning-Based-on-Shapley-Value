...
from src_opt.utils.geom_median import geometric_median_list_of_array
...
def solver():
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i in range(len(local_weights)):
            tmp = [local_weights[i][key] for key in local_weights[i].keys()]
            wlist.append(tmp)
...
if __name__ == '__main__':
    ...
    for _ in range(repeat):
        ...
        train_acc += train
        # show_avg(acc_list)
        path = '../save_opt/{}/RFA_{}_cnn_E{}_N{}_repeat{}_{}.txt'.format(NoiseWord[noise], args.dataset, args.epochs,  args.noiselevel, repeat, args.device)
        f = open(path, "a+")
